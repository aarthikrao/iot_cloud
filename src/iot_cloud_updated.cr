...
db2 = DB.open("sqlite3:./database2.db")
table_exists2 = db2.scalar "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='gps_data'"

if table_exists2 == 0
  puts "Creating GPS DATA table"
  db.exec "CREATE TABLE gps_data (lat int,lon int)"
